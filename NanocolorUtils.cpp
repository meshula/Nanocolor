...
NcCIEXYZ NcKelvinToXYZ(float temperature, float luminosity) {
    ...
    float cx = 0, cy = 0;
    if( 1667 <= t && t <=  4000)
        cx = -0.2661239e9 / t3 - 0.2343589e6 / t2 + 0.8776956e3 / t + 0.179910;
    else if( 4000 <  t && t <= 25000)
        cx = -3.025849e9  / t3 + 2.1070379e6 / t2 + 0.2226347e3 / t + 0.240390;

    float cx2 = cx * cx;
    float cx3 = cx2 * cx;
    if( 1667 <= t && t <=  2222)
        cy = -1.1063814 * cx3 - 1.34811020 * cx2 + 2.18555832 * cx - 0.20219683;
    else if( 2222 <  t && t <=  4000)
        cy = -0.9549476 * cx3 - 1.37418593 * cx2 + 2.09137015 * cx - 0.16748867;
    else if( 4000 <  t && t <= 25000)
...
NcCIEXYZ NcCIE1931ColorFromWavelength(float lambda, bool approx) {
    ...
    int i1 = (int) lambda;
    c1931 = (NcCIEXYZ) { xyz1931_1nm[i1][0], xyz1931_1nm[i1][1], xyz1931_1nm[i1][2] };

    if (i1 == 830-360)
        return normalize(c1931);
    
    NcCIEXYZ c2 = (NcCIEXYZ) { xyz1931_1nm[i1+1][0], xyz1931_1nm[i1+1][1], xyz1931_1nm[i1+1][2] };
